...
#---- grid_size = argv[1], robot_radius = argv[2], show_animation = argv[3], move_robot = argv[4], ArduinoSerialPort = argv[5], gx = argv[6], gy = argv[7], angle_orientation_finale = argv[8] -----#


#----Paramètres relatifs à l'algorithme de déplacement---
grid_size = float(sys.argv[1]) #0.25  # [m] # le pas de déplacement 0.25
robot_radius = float(sys.argv[2]) #0.5  # [m] # le rayon du robot  0.4
#---------------------------------------------------------
...
if(sys.argv[3]=="true"):
    ...
    show_animation = False # Si desactiver, la figure qui contient la carte 2D et le trajet va pas s'afficher

if(sys.argv[4]=="true"):
    move_robot = True # Si desactive, le robot ne va pas se déplacer
else:
    move_robot = False # Si desactive, le robot ne va pas se déplacer

ArduinoSerialPort = sys.argv[5] #Faut spécifier le port auquel est connecter l'Arduino ici
gx = float(sys.argv[6]) #-3.2 # [m] #coordonée suivant l'axe X de la position d'arrivée
gy = float(sys.argv[7]) #0 # [m] #coordonée suivant l'axe Y de la position d'arrivée
angle_orientation_finale = float(sys.argv[8]) #0 # [°] # angle d'orientation du robot une fois arrivé à destination
