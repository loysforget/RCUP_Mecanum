            ...
            for point in current_data:
                if point[0]==15:
                   if(point[1]>=240 or point[1]<=100):
                    obx.append(rx + (point[2]*np.sin((point[1] + angle)*pi/180))*0.001)
                    oby.append(ry + (point[2]*np.cos((point[1] + angle)*pi/180))*0.001)
    return [obx,oby]

def ObstacleProche(rx, ry, angle =0):
...
gx = 2  # [m] #coordonée suivant l'axe X de la position d'arrivée
gy = 8.2
 # [m] #coordonée suivant l'axe Y de la position d'arrivée
...
robot_radius = 0.4  # [m] #le rayon du robot
#---------------------------------------------------------
...
#-----Bloc 1-------------------------------------------------------------
    ...
    tmp1 = []
    tmp1 = update_map(xr, yr, angle_orientation)
    ox.extend(tmp1[0])
    oy.extend(tmp1[1]) 
            ...
            if move_robot:
                ...
                for j in range(len(rx)):
                   # m = m + 1
                    ...
                    tmp3 = ObstacleProche(xr, yr, angle_orientation)
                    Obstacle = tmp3[0]
                    ox.extend(tmp3[1])
                    oy.extend(tmp3[2]) 
