...
def ObjectifAtteint(pipe, gx, gy):
    tmp = cm.get_pose(pipe)
    x, y = tmp[0], tmp[1]
    return ((abs(gx - x)<=0.1) and (abs(gy - y)<= 0.1))
...
gy = 1 # [m] #coordonée suivant l'axe Y de la position d'arrivée
#--------------------------------------------

#----Paramètres relatifs à l'algorithme de déplacement---
grid_size = 0.3  # [m] #le pas de déplacement
...
#-----Bloc 2-------------------------------------------------------------
        ...
        """
        if(not(ObjectifAtteint(pipe, gx, gy))):
...
#-----Bloc 3-------------------------------------------------------------
            if move_robot:
                ...
                for j in range(len(rx)):
                    rx_c = str(rx[len(rx) - j - 1]*1000)
                    ry_c = str(str(ry[len(rx) - j - 1]*1000))
                    j = j +1
                    moveInAutoMode_x = "xAt"
                    moveInAutoMode_y = "yAt"
                    #moveInAutoMode_z = "zAt"
